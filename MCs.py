...
def detect_mcs(std_img, search_mask):
    ...
    print(f"\nThreshold value: {thresh_val:.1f} (90th percentile of search region)")
    ...
    show(mask, "Masked to Search Region")
    
    # AGGRESSIVE EROSION: Heavily shrink detected regions
    kernel_erode = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    mask = cv2.erode(mask, kernel_erode, iterations=1)
    show(mask, "After Strong Erosion (Heavily Shrunk)")
...
def analyze_detections(pred_mask, gt_mask):
    ...
    print("\n" + "="*60)
    print("WHERE ARE THE DETECTIONS?")
    print("="*60)
    if total > 0:
...
def get_mc_data(roi, radius=None):
    ...
    # GT region if radius is provided
    ...
    # Correct detections in yellow (if GT provided)
    if radius is not None:
