...
def compute_radius(mask, centroid):
    ...
    if not contours or centroid is None:
        return 0
    cnt = max(contours, key=cv2.contourArea)
...
def get_mass_data(image):
    """
    Input:
        image - 2D grayscale mammogram (numpy array)
    Output:
        mass_region - grayscale image masked by detected mass
        mass_mask   - binary mask of the mass
    """
    if image is None:
        mass_region = np.zeros((1024, 1024), dtype=np.uint8)
        mass_mask = np.zeros((1024, 1024), dtype=np.uint8)
